...
fun main(args: Array<String>) {
    ...
    tailrec fun <P1> onlyTrue(func: (P1) -> Boolean, list1: List<P1>, acc: List<P1> = listOf()): List<P1> = when {
        list1.isEmpty() -> acc
        else -> {
            var zipList = acc
            if (func(list1.head())) { zipList = zipList + list1.head() }
            onlyTrue(func, list1.tail(), zipList)
        }
    }
    ...
    val myResult = onlyTrue(above, myList)
    println(myResult)
}